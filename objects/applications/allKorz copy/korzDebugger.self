...
'.
 ...
 bootstrap addSlotsTo: bootstrap stub -> 'globals' -> 'korz' -> 'ide' -> 'korzProcess' -> () From: ( | {
         'ModuleInfo: Module: korzDebugger InitialContents: InitializeToExpression: (\'causeOfBirth in korzProcess has not been set\')'
        
         causeOfBirth <- 'causeOfBirth in korzProcess has not been set'.
        } | ) 
...
'.
 ...
 bootstrap addSlotsTo: bootstrap stub -> 'globals' -> 'korz' -> 'ide' -> 'korzProcess' -> 'parent' -> () From: ( | {
         ...
         activationStackLimit: lim IfFail: failBlk = ( |
            ...
            (((selfProcess stack
              filterBy: [|:a| a isKorzActivation] Into: list copyRemoveAll) asVector)
              copySize: lim)
 ...
 bootstrap addSlotsTo: bootstrap stub -> 'globals' -> 'korz' -> 'ide' -> 'korzProcess' -> () From: ( | {
         ...
         status <- bootstrap stub -> 'globals' -> 'processStatus' -> 'ready' -> ().
        } | ) 
...
\x7fIsComplete: '.
 ...
 bootstrap addSlotsTo: bootstrap stub -> 'traits' -> 'mirrors' -> 'activation' -> 'liveOnes' -> () From: ( | {
         ...
         isKorzActivation = ( |
            | 
            [0 evaluateForContextWithArguments: 0]. "browsing"
            (selector = 'evaluateForContextWithArguments:')  &&
            [(methodHolderIfFail: [^ false]) reflectee isMessageSend]).
        } | ) 
