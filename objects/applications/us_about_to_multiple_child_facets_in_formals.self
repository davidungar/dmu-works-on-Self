...
'.
 ...
 bootstrap addSlotsTo: bootstrap stub -> 'globals' -> 'us' -> 'formal' -> 'parent' -> () From: ( | {
         ...
         copyIfNeededToChangeFacetFrom: old To: new = ( |
            | 
            old == facet ifFalse: [self] True: [copy facet: new]).
        } | ) 
...
'.
 ...
 bootstrap addSlotsTo: bootstrap stub -> 'globals' -> 'us' -> 'slots' -> 'slot' -> 'parent' -> () From: ( | {
         ...
         addFormal: aFormal = ( |
            ...
            aFormal isDispatched ifTrue: [
              aFormal facet addUsParentSlot: self
            ].
            self).
        } | ) 
 ...
 bootstrap addSlotsTo: bootstrap stub -> 'globals' -> 'us' -> 'slots' -> 'slot' -> 'parent' -> () From: ( | {
         ...
         addMeInToDispatchedFormalsUsParentSlots = ( |
            | 
            dispatchedFormals do: [|:f|  f facet addUsParentSlot: self].
            self).
        } | ) 
...
'.
 ...
 bootstrap addSlotsTo: bootstrap stub -> 'shell' -> () From: ( | {
         ...
         todo = '
deposit fails for copied account because the depositSlot has formals that say what facet it is a parent of.
when facet is copied, old deposit slot then fails.
Do slots need to know all child facets?'.
        } | ) 
